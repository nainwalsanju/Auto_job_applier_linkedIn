            ...
            try:
                ...
                if (
                    ...
                    and (
                        ...
                        - getattr(print_lg, "_last_alert_time", 0)
                    )
                    > 30
                ):
...
def cleanup_artifacts(
    directory: str = "debug_html_dumps", max_files: int = 100, max_age_hours: int = 48
) -> None:
    """
    Maintenance function to prune temporary artifacts and debug files.
    - directory: The directory to clean.
    - max_files: Maximum number of most recent files to keep.
    - max_age_hours: Maximum age of files to keep (will still respect max_files).
    """
    try:
        if not os.path.exists(directory):
            return

        # Get all files with their full paths and modification times
        files = []
        ...
        for i, (path, mtime) in enumerate(files):
            # Delete if beyond max_files OR beyond max_age (except if it's within the top 5 most recent)
            if i >= max_files or (mtime < cutoff and i > 5):
                try:
                    os.remove(path)
                except Exception:
                    pass
    except Exception as e:
        print_lg(f"Maintenance Error: Failed to cleanup {directory}: {e}")

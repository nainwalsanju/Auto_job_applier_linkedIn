...
from typing import Literal, Any, List, Dict, Tuple, Optional
...
def _ensure_modal_closed(driver: Any) -> None:
    """
    ...
    try:
        ...
        for attempt in range(5):
            try:
                ...
                try:
                    modal_backdrops = driver.find_elements(
                        By.XPATH,
        ...
        try:
            message_modals = driver.find_elements(
                By.XPATH,
    ...
    try:
        remaining_modals = driver.find_elements(
            By.XPATH,
            "//div[contains(@class, 'artdeco-modal') or contains(@class, 'msg-overlay-conversation-bubble--is-active') or contains(@class, 'modal-backdrop')]",
        )
        # Only count actually visible modals
        visible_modals = [m for m in remaining_modals if m.is_displayed()]
        if visible_modals:
            print_lg(
                f"⚠️ Warning: {len(visible_modals)} visible modal(s) may still be present"
            )
        else:
...
def find_recruiters_on_job_page(driver: Any) -> list[dict]:
    """
    ...
    try:
        ...
        log_html_snapshot(
            driver,
            "recruiter_search_start",
            "Initial state before searching for recruiters",
        )
        ...
        try:
            hiring_team_header = driver.find_element(
                By.XPATH,
                "//h2[contains(@class, 'text-heading-medium') and contains(normalize-space(.), 'Meet the hiring team')]",
            )
            hiring_team_section = hiring_team_header.find_element(
            ...
            recruiter_info = _extract_recruiter_from_section(
                driver, hiring_team_section, "hiring_team"
            )
            if recruiter_info:
        ...
        try:
            connections_section = driver.find_element(
                By.XPATH,
            ...
            connections_list = find_connections_in_help_section(
                driver, connections_section
            )
            recruiters.extend(connections_list)

        except NoSuchElementException:
            print_lg(
                "No 'People you can reach out to' section found on this job posting."
            )
        ...
        try:
            ...
            if modal_connections:
                ...
                new_connections = [
                    c
                    for c in modal_connections
                    if c.get("recruiter_id") not in existing_ids
                ]
                recruiters.extend(new_connections)
                print_lg(
                    f"Added {len(new_connections)} new connections from 'Show all' modal"
                )
        except Exception as e:
...
def _extract_recruiter_from_section(
    driver: Any, section: WebElement, section_type: str
) -> dict | None:
    """
        ...
        try:
            ...
            for prefix in ["Message ", "Connect "]:
                if aria_label.startswith(prefix):
                    ...
                    print_lg(
                        f"📝 Extracted name from button aria-label: {recruiter_info['name']}"
                    )
                    break
        except:
            pass
    ...
    print_lg(
        f"Found {section_type} target: {recruiter_info['name']} ({recruiter_info['title']}) - "
...
def find_connections_in_help_section(
    driver: Any, connections_section: WebElement
) -> list[dict]:
    """
    ...
    try:
        ...
        connection_cards = connections_section.find_elements(
            By.XPATH,
...
def find_connections_in_show_all_modal(driver: Any) -> list[dict]:
    """
    ...
    try:
        ...
        driver.execute_script(
            "arguments[0].scrollIntoView({block: 'center'});", show_all_btn
        )
        buffer(0.5)
        ...
        connection_cards = modal.find_elements(
            By.XPATH,
            ".//div[contains(@class, 'artdeco-entity-lockup') and .//button[.//span[text()='Message']]]",
        )

        print_lg(
            f"📋 Found {len(connection_cards)} connections with Message button in modal"
        )
        ...
        for card in connection_cards:
            try:
                ...
                conn_info["message_type"] = (
                    "connection" if degree == "1st" else "inmail"
                )

                # Only add if it's a free message (1st degree)
                if conn_info["is_free_message"] and conn_info["name"] != "Unknown":
                    print_lg(
                        f"   ✅ Modal connection: {conn_info['name']} ({degree}) - Free Message"
                    )
                    connections.append(conn_info)
                else:
                    print_lg(
                        f"   ⏭️ Skipping modal connection: {conn_info['name']} ({degree}) - Not free"
                    )
        ...
        print_lg(
            f"📊 Total free messageable connections from modal: {len(connections)}"
        )
...
def check_message_capability(driver: Any, section: WebElement) -> dict:
    """
    ...
    try:
        ...
        try:
            message_button = section.find_element(
                By.XPATH,
        ...
        try:
            connect_button = section.find_element(
                By.XPATH,
        ...
        degree_text = ""
        try:
            connection_degree = section.find_element(
        ...
        if message_button:
            ...
            try:
                ...
                if not is_locked:
                    try:
                        message_button.find_element(
                            By.XPATH, ".//*[contains(@data-test-icon, 'lock')]"
                        )
                        is_locked = True
                    except:
                        pass
            ...
            if is_locked:
                ...
                result["is_free_message"] = (
                    False  # Conservative - assume InMail until proven free
                )
                result["message_type"] = "unknown"  # Let post-click determine
                print_lg(
                    f"⚠️ Pre-click check: Cannot determine if message is free (no 1st degree indicator)"
                )
...
Company: {company_name}"""
            ...
            if ai_provider.lower() == "openai":
                why_interested = ai_answer_question(
                    aiClient, interest_prompt, question_type="text"
                )
            elif ai_provider.lower() == "deepseek":
                why_interested = deepseek_answer_question(
                    aiClient, interest_prompt, question_type="text"
                )
            elif ai_provider.lower() == "gemini":
                why_interested = gemini_answer_question(
                    aiClient, interest_prompt, question_type="text"
                )
    ...
    subject = message_subject.format(
        job_title=job_title, company_name=company_name, recruiter_name=recruiter_name
    )
...
def send_message_to_recruiter(
    driver: Any, recruiter_info: dict, message_init_data: dict
) -> tuple[bool, str]:
    """
    ...
    log_html_snapshot(
        driver,
        f"msg_start_{recruiter_info.get('name', 'unknown')[:20]}",
    ...
    log_step(
        "Captured initial browser state",
        f"Window: {original_window[:30]}... | Handles: {len(pre_click_handles)}",
    )
    ...
    try:
        ...
        conversation_bubbles = driver.find_elements(
            By.XPATH,
        ...
        list_bubbles = driver.find_elements(
            By.XPATH,
    ...
    try:
        ...
        if section_type == "modal_connections":
            ...
            if not show_all_btn:
                return (
                    False,
                    f"Could not find 'Show all' button to message modal connection {current_name}",
                )
        ...
        if not action_button:
            raise Exception(
                f"Could not find '{button_type}' button for recruiter '{current_name}' (ID: {recruiter_id})"
            )
        ...
        driver.execute_script(
            "arguments[0].scrollIntoView({block: 'center'});", action_button
        )
        buffer(1)
        action_button.click()
        buffer(2)
    ...
    if recruiter_info["button_type"] == "connect":
        try:
            ...
            modal_xpath = (
                "//div[contains(@class, 'artdeco-modal') and contains(., 'Invite')]"
            )
            WebDriverWait(driver, 5).until(
            ...
            try:
                ...
                print_lg(
                    "ℹ️ 'Add a note' button not found, checking if text area is already visible"
                )
            ...
            try:
                ...
                if len(entered_text.strip()) < len(message_body) * 0.5:
                    print_lg(
                        "⚠️ Note field insert check failed; retrying with JS + send_keys"
                    )
            ...
            recruiter_key = (
                f"{recruiter_info.get('recruiter_id', 'unknown')}:{action_type}"
            )
            ...
            if recruiter_info.get("has_message_button"):
                ...
                return send_message_to_recruiter(
    ...
    print_lg(
        f"🔍 Strictly verifying conversation context for {recruiter_info['name']}..."
    )
    ...
    try:
        if is_new_window:
            ...
            if recruiter_first_name in page_source_lower:
                ...
                print_lg(
                    f"✅ Verified - Recruiter first name '{recruiter_first_name}' found in new window"
                )
            else:
                ...
                if name_prefix in page_source_lower:
                    verified_context = True
                    print_lg(
                        f"✅ Verified - Recruiter name prefix '{name_prefix}' found in new window"
                    )
                else:
                    # Check URL for profile indicator
                    if (
                        "/messaging/" in driver.current_url
                        or "/in/" in driver.current_url
                    ):
                        print_lg(
                            f"✅ Verified - Messaging URL detected, proceeding with message"
                        )
                        verified_context = True
                    else:
                        print_lg(
                            f"⚠️ New window verification failed - name '{recruiter_first_name}' not found in page"
                        )
        else:
            ...
            for verification_xpath in verification_xpaths:
                try:
                    WebDriverWait(driver, 3).until(
                        EC.presence_of_element_located((By.XPATH, verification_xpath))
                    )
                    print_lg(
                        f"✅ Verified - Recruiter name '{recruiter_first_name}' found in active conversation"
                    )
                    verified_context = True
                    break
            ...
            if not verified_context:
                ...
                if recruiter_first_name.lower() in driver.page_source.lower():
                    verified_context = True
                    print_lg(
                        f"✅ Verified (Page Source) - Name '{recruiter_first_name}' found"
                    )
    ...
    if not verified_context:
        ...
        try:
            ...
            return (
                False,
                "SAFETY ABORT: Generic 'New message' window detected (No Recipient)",
            )
        except:
            pass
        ...
        return (
            False,
            f"SAFETY ABORT: Could not verify that message window belongs to {recruiter_info['name']}",
        )
    ...
    if recruiter_info.get("connection_degree") == "1st":
        print_lg(
            f"✅ Skipping InMail check - {recruiter_info['name']} is 1st degree connection (always free)"
        )
        is_confirmed_free = True
    elif recruiter_info.get("is_free_message", False):
        print_lg(
            f"✅ Skipping InMail check - {recruiter_info['name']} was pre-verified as free message"
        )
        is_confirmed_free = True
    else:
        ...
        print_lg(
            f"⚠️ {recruiter_info['name']} is NOT 1st degree - checking for InMail indicators..."
        )
        ...
        try:
                ...
                try:
                    ...
                    for elem in elems:
                        if elem.is_displayed():
                            ...
                            print_lg(
                                f"⚠️ InMail detected via VISIBLE element: {indicator[:60]}..."
                            )
                            break
                    if is_inmail_modal:
                        break
                ...
                try:
                    ...
                    for elem in elems:
                        if elem.is_displayed():
                            ...
                            is_inmail_modal = False
                            print_lg(
                                f"✅ Found FREE indicator - confirmed as free message"
                            )
                            break
            ...
            if not is_confirmed_free and not is_inmail_modal:
                ...
                print_lg(
                    f"⚠️ SAFETY CHECK: Pre-click detection marked as NOT free, no positive free confirmation found"
                )
    ...
    if is_inmail_modal:
        ...
        if not modal_closed:
            print_lg(
                "⚠️ Could not close InMail modal - will be cleaned up by next modal check"
            )
    ...
    try:
        ...
        print_lg(
            f"🧠 Generating AI message for {recruiter_info['name']} (Verified Free)..."
        )
        ...
        driver.execute_script(
            "arguments[0].scrollIntoView({block: 'center'});", msg_body
        )
        ...
        try:
            ...
            if len(entered_text) < 10:
                print_lg(
                    f"⚠️ Warning: Only {len(entered_text)} chars inserted. Trying Actions chain..."
                )
                # Fallback: ActionChains
                actions = ActionChains(driver)
                actions.move_to_element(msg_body).click().send_keys(
                    message_body
                ).perform()
                buffer(0.5)
                entered_text = msg_body.get_attribute("innerText").strip()
    ...
    try:
        ...
        if send_verified:
            ...
            print_lg(
                f"⚠️ Message clicked send for {recruiter_info['name']} but modal still visible"
            )
...
def check_daily_message_limit() -> bool:
    ...
    if messages_sent_today >= max_messages_per_day:
        print_lg(
            f"⚠️ Daily message limit reached ({max_messages_per_day}). Stopping message sending."
        )
        return True
...
def should_skip_recruiter(
    ...
    if (
        skip_inmail_required
        and not recruiter_info["is_free_message"]

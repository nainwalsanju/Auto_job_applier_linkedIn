    ...
    * Returns an OpenAI-compatible client configured for DeepSeek
    ...
    try:
        ...
        if not ai_config.use_AI:
            raise ValueError(
                "AI is not enabled! Please enable it by setting `USE_AI=true` in your .env file or environment variables."
            )

        base_url = ai_config.llm_api_url

        if base_url.endswith("/"):
            base_url = base_url[:-1]

        # Create client with DeepSeek endpoint
        client = OpenAI(base_url=base_url, api_key=ai_config.llm_api_key)
    ...
    * Returns the response as text or JSON
    ...
    try:
        ...
        if stream:
            ...
            for chunk in completion:
                ...
                if chunk.model_extra and chunk.model_extra.get("error"):
                    raise ValueError(
                        f'Error occurred with DeepSeek API: "{chunk.model_extra.get("error")}"'
                    )
            ...
            if completion.model_extra and completion.model_extra.get("error"):
                raise ValueError(
                    f'Error occurred with DeepSeek API: "{completion.model_extra.get("error")}"'
                )
        ...
        if "Connection" in str(e):
            ...
            print_lg(
                "If you're behind a firewall or proxy, make sure it allows connections to DeepSeek API."
            )
        elif "401" in str(e):
            print_lg(
                "This appears to be an authentication error. Your API key might be invalid or expired."
            )
        elif "404" in str(e):
            print_lg(
                "The requested resource could not be found. The API URL or model name might be incorrect."
            )
        elif "429" in str(e):
            print_lg(

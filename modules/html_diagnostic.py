...
def save_job_page_html(driver: WebDriver, job_id: str, job_title: str, company: str):
    ...
    try:
        ...
        with open(analysis_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"\n{'='*60}\n\n")
            f.write(f"CHECKLIST - Does HTML contain:\n")
            f.write(f"  [{'✓' if has_meet_team else 'X'}] 'Meet the hiring team' text\n")
            f.write(f"  [{'✓' if has_heading_medium else 'X'}] 'text-heading-medium' class\n")
            f.write(f"  [{'✓' if has_artdeco_card else 'X'}] 'artdeco-card' class\n")
            f.write(f"  [{'✓' if has_jobs_poster_name else 'X'}] 'jobs-poster__name' class\n")
            f.write(f"  [{'✓' if has_entry_point else 'X'}] 'entry-point' class\n")
            f.write(f"\n{'='*60}\n\n")
            
            if has_meet_team:
                f.write("✅ HTML HAS 'Meet the hiring team' section!\n")
                f.write("If XPath still fails, there's a selector bug.\n")
            else:
                f.write("❌ HTML does NOT have 'Meet the hiring team' section.\n")

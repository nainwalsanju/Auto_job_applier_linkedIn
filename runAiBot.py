        ...
        if select:
            ...
            if overwrite_previous_answers or selected_option == "Select an option":
                ...
                # Add location handling
                    ...
                    loc_word in label for loc_word in ["location", "city", "state", "country"]
                ):
                    if "country" in label:
                        answer = country
                    elif "state" in label:
        ...
        if text:
            ...
            if not prev_answer or overwrite_previous_answers:
                if "experience" in label or "years" in label:
                    ...
                    "salary" in label or "compensation" in label or "ctc" in label or "pay" in label
                ):
                    ...
                    "website" in label or "blog" in label or "portfolio" in label or "link" in label
                ):
                    answer = website
    ...
    if easy_apply_only:
        try:
            if (
                ...
                in driver.find_element(By.CLASS_NAME, "artdeco-inline-feedback__message").text
            ):
...
def main() -> None:
    try:
        ...
        print_lg("                              ----------")
        print_lg(
            "Total applied or collected:     {}".format(easy_applied_count + external_jobs_count)
        )
